...
export function SkillsPanel() {
  const { player, setPlayer, setNpcLevel } = useStore();
        ...
        <div className="space-y-2">
          {displayedNpcs.map((npc) => (
            <div key={npc} className="flex items-center gap-2">
              <span className="text-xs text-slate-300 w-36 truncate">{npc}</span>
              <input
                type="number"
                min={0}
                max={20}
                ...
                className="w-16 bg-slate-700 border border-slate-600 rounded px-2 py-0.5 text-sm text-slate-100 focus:outline-none focus:border-purple-500"
              />
            </div>
          ))}
        </div>
